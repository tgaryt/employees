...
// Set page title
$pageTitle = 'Edit Employee: ' . htmlspecialchars($employee['first_name'] . ' ' . $employee['last_name']);

// Initialize variables with employee data
...
$workHours = $employee['work_hours_per_day'];
$workSchedule = $employee['work_schedule'];
$pstWorkHours = $employee['pst_work_hours'];
$passportNumber = $employee['passport_number'];
$localCellNumber = $employee['local_cell_number'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if (!isset($_POST['csrf_token']) || !verifyCSRFToken($_POST['csrf_token'])) {
		...
		// If no errors, update employee in database
		if (empty($errors)) {
			try {
				$stmt = $pdo->prepare("
					UPDATE employees SET
						first_name = ?, last_name = ?, email = ?, job_position = ?, 
						salary = ?, currency = ?, location = ?, residential_address = ?,
						payment_method = ?, employment_type = ?, work_hours_per_day = ?, 
						work_schedule = ?, pst_work_hours = ?, passport_number = ?,
				...
				$stmt->execute([
					...
					$localCellNumber, $emergencyContact, $startDate, $employeeId
				]);
				...
				if (!empty($_FILES['offer_letter']['name'])) {
					...
					if ($uploadResult['success']) {
						...
						if (documentExists($pdo, $employeeId, 'offer_letter')) {
							// Update existing document
							$stmt = $pdo->prepare("
								UPDATE employee_documents SET
									file_name = ?, file_path = ?, upload_date = NOW()
				...
				if (!empty($_FILES['id_front']['name'])) {
					...
					if ($uploadResult['success']) {
						// Check if document already exists
						if (documentExists($pdo, $employeeId, 'id_front')) {
							// Update existing document
							$stmt = $pdo->prepare("
								UPDATE employee_documents SET
									file_name = ?, file_path = ?, upload_date = NOW()
				...
				if (!empty($_FILES['id_back']['name'])) {
					...
					if ($uploadResult['success']) {
						// Check if document already exists
						if (documentExists($pdo, $employeeId, 'id_back')) {
							// Update existing document
							$stmt = $pdo->prepare("
								UPDATE employee_documents SET
									file_name = ?, file_path = ?, upload_date = NOW()
				...
				if (empty($errors)) {
					// Redirect to employee view page with success message
					header("Location: view.php?id=$employeeId&success=updated");
					exit;
				}
	...
	<div class="p-8">
		<div class="flex justify-between items-center mb-6">
			...
			<div class="flex space-x-3">
				<a href="view.php?id=<?php echo $employeeId; ?>" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition duration-150">
		...
		<div class="bg-white rounded-lg shadow-lg overflow-hidden">
			...
			<form action="edit.php?id=<?php echo $employeeId; ?>" method="post" enctype="multipart/form-data" class="p-6">
				...
				<div data-tab-content="documents" class="hidden">
					<!-- Current Documents -->
					<div class="mb-6">
						<h3 class="text-sm font-medium text-gray-700 mb-2">Current Documents</h3>
										...
										<li class="py-3 flex items-center justify-between">
											<div class="flex items-center">
												...
												<?php elseif ($document['document_type'] === 'id_back'): ?>
													...
													<span>ID Back</span>
												<?php endif; ?>
												<span class="text-xs text-gray-500 ml-2">(<?php echo htmlspecialchars($document['file_name']); ?>)</span>
											</div>
											<div>
						...
						<?php else: ?>
							<p class="text-gray-500 text-sm bg-gray-50 rounded-md p-4 mb-4">No documents uploaded yet.</p>
						<?php endif; ?>
					</div>
				...
				<div class="border-t border-gray-200 pt-6 mt-6">
					<div class="flex justify-end space-x-3">
						<a href="view.php?id=<?php echo $employeeId; ?>" class="px-4 py-2 bg-gray-200 text-gray-700 rounded hover:bg-gray-300 transition duration-150">
							Cancel
						</a>
